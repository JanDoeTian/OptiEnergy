...
export function SiteItem({site, onView, onEdit, onDelete}: Props) {
    ...
    return (
        <>
            ...
            <Stack sx={{ p: 3, pb: 2 }} direction="row" alignItems="top" spacing={2} marginRight={3} maxHeight={120}>
        ...
        </Stack>            <Stack sx={{ p: 3, pb: 2 }} direction="row" alignItems="top" spacing={2} marginRight={3}>
        ...
        {site.messages.map((message) => {
            const daysAgo = Math.floor((new Date().getTime() - new Date(message.createdAt).getTime()) / (1000 * 60 * 60 * 24));
            return (
                <Stack flexDirection="row" alignItems="center" justifyContent="space-between" marginRight={1}>
                    <Alert severity={message.type} sx={{backgroundColor: 'transparent', maxWidth: 290}}>
                        {message.message}
                    </Alert>
                    <Typography variant="caption" color="text.disabled">
                        {daysAgo} days ago
